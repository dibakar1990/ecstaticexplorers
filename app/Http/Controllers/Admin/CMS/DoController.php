...
class DoController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function($row){
                    $editUrl = route('admin.do.edit',['do' => Crypt::encrypt($row->id) ]);
                    $deleteUrl = route('admin.do.destroy',['do' => $row->id]);
                    $actionButton ='<div class="d-flex">
                        <a href="' .$editUrl. '" class="btn btn-sm btn-outline-primary px-1"><i class="bx bxs-edit"></i></a>&nbsp;
                        <a href="javascript:;" class="ms-1 btn btn-sm btn-outline-danger px-1" data-bs-target="#deleteConfirm" data-bs-toggle="modal" onclick="deleteConfirm(\''. $deleteUrl .'\', \'Are you sure, want to delete?\', \'Delete Do Confirmation\')"><i class="bx bxs-trash"></i></a>
                    </div>';
                    return $actionButton;
                })
        ...
        return view('admin.cms.do.index');
    }

    public function create()
    {
        return view('admin.cms.do.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $do = new CmsDo();
        $do->title = $request->name;
        $do->save();
        $redirect = route('admin.do.index');
        return $this->success($redirect, 'Cms do created successfully');
    }

    public function edit(string $id)
    {
        $do_id = Crypt::decrypt($id);
        $item = CmsDo::findOrFail($do_id);
        return view('admin.cms.do.edit',compact('item'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $do = CmsDo::findOrFail($id);
        $do->title = $request->name;
        $do->save();
        $redirect = route('admin.do.index');
        return $this->success($redirect, 'Cms do updated successfully');
    }
    ...
    public function destroy(string $id)
    {
        CmsDo::destroy($id);
        $redirect = route('admin.do.index');
        return $this->success($redirect, 'Cms do deleted successfully');
    }

    public function status(Request $request)
    {
        $response = CmsDo::findOrFail($request->id);
        $response->status = $request->status;
        $response->save();
        if($response){
            return $this->ajaxSuccess($response, 'Cms do status has been changed successfully');
        }else{
    ...
    public function action(Request $request)
    {
        if($request->action_value == 'is_delete'){
            ...
            $redirect = route('admin.do.index');
            return $this->ajaxSuccess(['response' => $redirect, 'success_msg' => 'Cms do deleted successfully','status' => true]);
        }
    }
}
