...
class LanguageController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function($row){
                    $editUrl = route('admin.language.edit',['language' => Crypt::encrypt($row->id) ]);
                    $deleteUrl = route('admin.language.destroy',['language' => $row->id]);
                    $actionButton ='<div class="d-flex">
                        <a href="' .$editUrl. '" class="btn btn-sm btn-outline-primary px-1"><i class="bx bxs-edit"></i></a>&nbsp;
                        <a href="javascript:;" class="ms-1 btn btn-sm btn-outline-danger px-1" data-bs-target="#deleteConfirm" data-bs-toggle="modal" onclick="deleteConfirm(\''. $deleteUrl .'\', \'Are you sure, want to delete?\', \'Delete Language Confirmation\')"><i class="bx bxs-trash"></i></a>
                    </div>';
                    return $actionButton;
                })
    ...
    public function store(Request $request)
    {
        ...
        return $this->success($redirect, 'Language created successfully');
    }

    public function edit(string $id)
    {
        $language_id = Crypt::decrypt($id);
        $item = Language::findOrFail($language_id);
    ...
    public function update(Request $request, string $id)
    {
        ...
        $language->save();
        $redirect = route('admin.language.index');
        return $this->success($redirect, 'Language updated successfully');
    }
    ...
    public function destroy(string $id)
    {
        Language::destroy($id);
        $redirect = route('admin.language.index');
        return $this->success($redirect, 'Language deleted successfully');
    }

    public function status(Request $request)
    {
        $response = Language::findOrFail($request->id);
        $response->status = $request->status;
        $response->save();
        if($response){
            return $this->ajaxSuccess($response, 'Language status has been changed successfully');
        }else{
    ...
    public function action(Request $request)
    {
        if($request->action_value == 'is_delete'){
            ...
            $redirect = route('admin.language.index');
            return $this->ajaxSuccess(['response' => $redirect, 'success_msg' => 'Language deleted successfully','status' => true]);
        }
    }
}
