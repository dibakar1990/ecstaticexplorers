<form class="row g-3" id="createForm" method="post" action="{{ route('admin.gallery.image.store',['id' => $item->id]) }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="col-md-12">
        <div class="row mb-3 row-cols-auto g-2 justify-content-center mt-3 preview-images-zone">
            @foreach($item->gallery_images as $rowImage)
            @if($rowImage->file_path !=='')
                <div class="col">
                    <img src="{{ $rowImage->file_path_url }}" width="100" height="100" class="border rounded cursor-pointer" alt="">
                    <a href="javascript:;" class="delete-image" data-gallery-id="{{ $item->id }}" data-action-url="{{route('admin.gallery.delete.image',['id'=> $rowImage->id])}}"><span class="icon"><i class="fas fa-trash-alt" style="color: red"></i></span></a>
                    <div class="form-check">
                        <input class="form-check-input default-image" type="radio" data-action-url="{{route('admin.gallery.default.image.set',['id'=> $item->id])}}" name="default_status" value="{{$rowImage->id}}" id="flexRadioDefault{{$rowImage->id}}" @if($rowImage->default_status == 1){{'checked'}}@endif>
