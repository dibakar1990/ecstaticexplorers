...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            ...
            <div class="ms-auto">
                <div class="btn-group">
                    <a href="{{ route('admin.states.create') }}" class="btn btn-sm btn-primary px-3 radius-30">Add New</a>
                </div>
            </div>
        </div>
        <!--end breadcrumb-->
        ...
        <div class="row">
            ...
            <div class="col-xxl-2 col-lg-2 mb-3 filter-trending">
                <div class="form-group">
                    <select class="single-select" id="filter_trending" name="trending">
                        <option value="">{{ __('Trending Search') }}</option>
                        <option value="1">{{ __('Yes') }}</option>
                        <option value="0">{{ __('No') }}</option>
                    </select>
                </div>
            </div>
            <div class="col-xxl-2 col-lg-2 mb-3 filter-explore-state">
                <div class="form-group">
                    <select class="single-select" id="filter_explore_state" name="explore_state">
                        <option value="">{{ __('Explore by States') }}</option>
                        <option value="1">{{ __('Yes') }}</option>
                        <option value="0">{{ __('No') }}</option>  
                    </select> 
                </div>   
            </div>
            <div class="col-xxl-3 col-lg-3 mb-3 filter-explore-unexplore">
                <div class="form-group">
                    <select class="single-select" id="filter_explore_unexplore" name="explore_unexplore">
                        <option value="">{{ __('Explore the Unexplored') }}</option>
                        <option value="1">{{ __('Yes') }}</option>
                        <option value="0">{{ __('No') }}</option>  
                    </select> 
                </div>   
        ...
        <div class="card border-top border-0 border-4 border-primary">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="dataTable" class="table table-striped table-bordered" style="width:100%">
                        <thead>
                            <tr>
                                ...
                                <th>Name</th>
                                <th>Trending</th>
                                <th>Explore by States</th>
                                <th>Explore the Unexplored</th>
                                <th>Status</th>
                                <th>Action</th> 
                            </tr>
                        </thead>
                    ...
                    <input type="hidden" class="statusActionUrl" value="{{route('admin.state.status')}}">
                    <input type="hidden" class="actionUrl" value="{{route('admin.state.action')}}">
                </div>
            </div>
        </div>
    </div>
...
<script>
    $(function () {
        ...
        var table = $('#dataTable').DataTable({
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    d.trending = $('#filter_trending').val(),
                    d.type = $('#filter_explore_state').val(),
                    d.type = $('#filter_explore_unexplore').val(), 
            ...
            columns: [
                ...
                {data: 'explore_state', name: 'explore_state', orderable: false, searchable: false},
                {data: 'explore_unexplore', name: 'explore_unexplore', orderable: false, searchable: false}, 
        ...
        $('#filter_trending').change(function(){
            table.draw();
        });
        $('#filter_explore_state').change(function(){
            table.draw();
        });  
        $('#filter_explore_unexplore').change(function(){
            table.draw();
        });    
