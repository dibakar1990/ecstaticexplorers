            ...
            <div class="col-xl-12 mx-auto">
                ...
                <div class="card">
                    <div class="card-body p-4">
                        <form class="row g-3" id="editForm" method="post" action="{{ route('admin.blogs.update',['blog' => $item->id]) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="col-md-12 required">
                                <label for="title" class="form-label">Title</label>
                                <input type="text" class="form-control" id="title" name="title" value="{{ $item->title }}" placeholder="Blog title">
                                @error('title')
                            ...
                            <div class="col-md-12 required">
                                ...
                                <input type="text" class="form-control" id="text" name="text" value="{{ $item->text }}" placeholder="Blog text">
                                @error('text')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <div class="col-md-12 required">
                                ...
                                <select class="form-select single-select" id="blog_category_id" placeholder="Choose category" name="blog_category_id">
									...
									<option value="{{ $category->id }}" @if($item->blog_category_id == $category->id) {{ 'selected' }}@endif>{{ $category->name }}</option>
                                    @endforeach
								</select>
                            ...
                            <div class="col-md-12 required">
                                ...
                                <select class="form-select multiple-select-field" id="tag_id" placeholder="Choose tags" name="tag_id[]" multiple>
                                    ...
                                    @foreach($tags as $tag)
									<option value="{{ $tag->id }}" {{ $item->blog_tags->pluck('tag_id')->contains($tag->id) ? 'selected' : '' }}>{{ $tag->name }}</option>
                                    @endforeach
								</select>
                                @error('tag_id')
                                <span class="invalid-feedback" role="alert">
